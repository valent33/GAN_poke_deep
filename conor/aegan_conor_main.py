...
import aegan_conor_tools as tools
import aegan_conor_models as gm
...
if __name__ == '__main__':
    ...
    shape = (np.array((6, 6)) * 16)

    dirs = set_up_directories(f'sixth_AEGAN')
    data = tools.load_image_files("conor/sprites_rgb", reshape=shape, flip=True)
    x_train = data[:int(0.9*len(data))]
    x_test = data[int(0.9*len(data)):]
    del data
    
    # shutil.copytree(DIRECTORY, os.path.join(dirs['experiment'], 'code'))
    test_images, test_noise = set_up_test_data(dirs["results"], "sixth", 100, x_test)

    train_model(MODEL_TYPES['AEGAN'],
                dirs=dirs,
                plot_every=1,
                latent_dim=100,
                batch_size=32,
