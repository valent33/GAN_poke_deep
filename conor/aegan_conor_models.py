...
from keras.layers import (Input, Dense, Flatten, Concatenate,
                                     ...
                                     Conv2D, Activation, Dropout, LeakyReLU, ReLU,
...
class GenerativeModel(ABC):
    ...
    def decode(self, latent_vectors):
        ...
        return self.generator.predict(latent_vectors, verbose=0)

    def generate(self, num):
        """Generate `num` random samples."""
        return self.decode(self.noise_generating_function(num))

    def _build_feature_extractor(self, image_shape, output_width, discriminator=False):
        ...
        for filter in filters:
            X = Conv2D(filter, kernel_size=kernel_size, strides=2, padding='same', kernel_initializer=INIT)(X)
            ...
            if discriminator:
                ...
                X = Activation("relu")(X)
        X = Conv2D(128, kernel_size=kernel_size, strides=1, padding='same', kernel_initializer=INIT)(X)
        # X = GaussianDropout(0.005)(X)
        X = LayerNormalization()(X)
        if discriminator:
            X = LeakyReLU(0.02)(X)
            ...
            X = Activation("relu")(X)
        X = Conv2D(256, kernel_size=kernel_size, strides=1, padding='same', kernel_initializer=INIT)(X)
        # X = GaussianDropout(0.005)(X)
        X = LayerNormalization()(X)
        if discriminator:
            X = LeakyReLU(0.02)(X)
    ...
    def _build_generator(self, latent_dim):
        ...
        starting_shape = (6, 6, 64)

        input_layer = Input((latent_dim,))
        X = Dense(np.prod(starting_shape), kernel_initializer=INIT)(input_layer)
        X = LayerNormalization()(X)
        X = Activation("relu")(X)
        ...
        for filter in filters:
            ...
            X = UpSampling2D(2)(X)
            Y = UpSampling2D(2)(Y)
            X = Conv2D(filters=filter, kernel_size=4, strides=1, padding='same', kernel_initializer=INIT)(X)
            X = LayerNormalization()(X)
            X = Activation("relu")(X)
        X = Concatenate()([X, Y])
        X = UpSampling2D(1)(X)
        Y = UpSampling2D(1)(Y)
        X = Conv2D(filters=64, kernel_size=4, strides=1, padding='same', kernel_initializer=INIT)(X)
        X = LayerNormalization()(X)
        X = Activation("relu")(X)

        X = Concatenate()([X, Y])
        X = Conv2D(filters=3, kernel_size=4, strides=1, padding='same', kernel_initializer=INIT)(X)
    ...
    @abstractmethod
    def train(self, batch_size=32, batch_num=1, verbose=False, prepend=''):
        pass
...
class AutoEncoder(GenerativeModel):
    ...
    def __init__(self,
        ...
        super().__init__(image_shape,
                        ...
                        #  parameter_json_path,
                         data_generating_function,
                         noise_generating_function)
        self.encoder = self._build_encoder(self.image_shape, self.latent_dim)
        self.autoencoder_image = self._build_ae(self.encoder, self.generator, loss="mae", learning_rate=0.0005)


    def _build_encoder(self, image_shape, latent_dim):
        """Build a model that maps images to a latent space."""
        model = self._build_feature_extractor(image_shape=image_shape, output_width=latent_dim)
        return model

    def _build_ae(self, encoder, decoder, loss='mae', learning_rate=0.0005):
        ...
        model.compile(Adam(clipnorm=1, learning_rate=learning_rate), loss=loss)
        return model
    ...
    def autoencode_images(self, images):
        ...
        return self.autoencoder_image.predict(images, verbose=0)
...
class GenerativeAdversarialNetwork(GenerativeModel):
    ...
    def __init__(self,
        ...
        super().__init__(image_shape,
                        ...
                        #  parameter_json_path,
                         data_generating_function,
                         noise_generating_function)
        self.discriminator_image = self._build_discriminator_image(image_shape)
        self.gan_image = self._build_gan(self.generator, self.discriminator_image)


    def _build_discriminator_image(self, image_shape):
        """Build a model that classifies images as real (1) or fake (0)."""

        model = self._build_feature_extractor(image_shape=image_shape, output_width=1, discriminator=True)
        model.compile(optimizer=Adam(clipnorm=1, learning_rate=0.0002, beta_1=0.5), loss="binary_crossentropy")
        return model
    ...
    def discriminate_images(self, images):
        ...
        return self.discriminator_image.predict(images, verbose=0)
...
class AdversarialAutoEncoder(AutoEncoder):
    ...
    def __init__(self,
        ...
        super().__init__(image_shape,
                        ...
                        #  parameter_json_path,
                         data_generating_function,
                         noise_generating_function)
        self.discriminator_latent = self._build_discriminator_latent(latent_dim)
    ...
    def _build_discriminator_latent(self, latent_dim):
        ...
        output_layer = Activation('sigmoid')(X)
        model = Model(input_layer, output_layer)
        model.compile(Adam(clipnorm=1, learning_rate=0.0002, beta_1=0.5), loss="binary_crossentropy")
        # print(model.summary())
        return model
    ...
    def _build_aae(self, encoder, generator, discriminator):
        ...
        model.compile(Adam(clipnorm=1, learning_rate=0.0002, beta_1=0.5), loss=["mae", "binary_crossentropy"],
                      loss_weights=[10, 1]
                      )
        return model
...
class AutoEncodingGenerativeAdversarialNetwork(AdversarialAutoEncoder, EncodingGenerativeAdversarialNetwork):
    ...
    def _build_aegan(self, encoder, generator, discriminator_image, discriminator_latent):
        ...
        model.compile(Adam(clipnorm=1, learning_rate=0.0001, beta_1=0.5), loss=["mae", "mse", "binary_crossentropy", "binary_crossentropy", "binary_crossentropy", "binary_crossentropy"],
